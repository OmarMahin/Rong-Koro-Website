...
const Canvas = () => {
  ...
  return (
    <Flex
      className={
        "flex w-full h-[calc(100vh-250px)] md:mt-10 mt-6 mb-20 border-2 border-gray-200 rounded-2xl relative"
      }
      ref={flexRef}
    >
      <button
        className="absolute top-0 -translate-y-1/2 left-1/2 -translate-x-1/2 bg-green-400 hover:cursor-pointer py-2 px-4 border-none outline-none rounded-full z-30 font-semibold md:text-xl text-sm"
        onClick={() => setShowSelectonMenu(true)}
      >
        ছবি পরিবর্তন
      </button>
      <canvas
        ...
        onMouseUp={mouseUp}
        onMouseMove={(e) => painting && movment(e)}
      ...
      >
        <Flex
          ...
          onClick={() => setShowSelectonMenu(false)}
        >
